...
router.get('/', (req, res, next) => {
    ...
    getMenu(2).then(function (myResult) {
        getDataSubmenu().then(function (myDataSubmenu) {
            ...
            res.render('admin/submenu/index', {
...
router.get('/create', (req, res, next) => {
    ...
    getMenu(2).then(function (myResult) {
        getDataMenu().then(function (myMenu) {
            ...
            res.render('admin/submenu/create', {
                title: 'Submenu Management',
                sumber: '/../sistem',
                menu: myResult,
                data: myMenu,
...
router.post('/create', (req, res, next) => {
    ...
    client.query('INSERT INTO users_sub_menu(menu_id, title, url, icon, is_active, created_at) VALUES($1, $2, $3, $4, $5, $6)', cols, function (err, result) {
        if (err) {
            req.flash('alertMessage', err.message);
            req.flash('alertStatus', 'danger');
            console.log("Error Saving : %s ", err);
            res.redirect('/admin/submenu/create');
        }
        req.flash('alertMessage', 'Success Add New Menu');
        req.flash('alertStatus', 'success');
        req.session.isLogin = 'lewat';
        res.redirect('/admin/submenu');
    });
});
...
router.post('/:id/edit', (req, res, next) => {
    ...
    client.query('UPDATE users_sub_menu SET menu_id=$1, title=$2, url=$3, icon=$4, is_active=$5, created_at=$6 WHERE id=$7', cols, function (err, result) {
        if (err) {
            ...
            req.flash('alertStatus', 'danger')
            console.log("Error Saving : %s ", err);
            res.redirect(`/admin/submenu/${req.params.id}/edit`);
        }
